...
class build_discriminator(tf.keras.layers.Layer):
    def down_CNN(img_shape = (512,768,1),gt = 64):
        ...
        img_E = tf.keras.Input(shape=img_shape)
        ...
        return tf.keras.Model([img_B, img_E], validity)
...
def combined(generator, discriminator, img_shape = (512,768,1)):
    img_B = tf.keras.Input(shape=img_shape)
    img_E = tf.keras.Input(shape=img_shape)
    fake_E = generator(img_B)
    discriminator.trainable = False
    valid = discriminator([fake_E, img_B])
    return tf.keras.Model(inputs=[img_B, img_E], outputs=[valid, fake_E])
# if __name__ == '__main__':
