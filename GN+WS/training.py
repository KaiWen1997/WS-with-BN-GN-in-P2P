...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for steps_per_epoch in tqdm(range(steps_per_epochs)):
            ...
            '''
            # pdb.set_trace()
            imgs_B,imgs_E = next(data_gen)
            fake_E = model_g.predict(imgs_B)
            for D_times in range(10):
                d_loss_real = model_d.train_on_batch([imgs_E, imgs_B], valid)
                d_loss_fake = model_d.train_on_batch([fake_E, imgs_B], fake)
            d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
            '''
            Train Generator
            '''
            g_loss = combine.train_on_batch([imgs_B, imgs_E], [valid, imgs_E])
